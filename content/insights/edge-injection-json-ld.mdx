...
excerpt: "既存ECサイトのソースコードを変更せず、Cloudflare WorkersのHTMLRewriterでProduct、Offer、BreadcrumbListのJSON-LDを全ページへ注入する実装を解説します。"
...
**May 2026 Core Update（2026-05-21 開始 → 2026-06-02 完了、Google Search Status Dashboard）を経て、Edge Injection の位置づけを一段下げて説明するようになりました。**

本文で書いたとおり、既存サイトを改修せずに構造化データを全ページへ行き渡らせる手段としては、いまも有効です。ただし **Edge Injection で埋められるのは「機械可読性」までで、「内容の独自性」は埋まりません。** Core Update の連続適用で下げられたのは、スキーマが無いページというより、スキーマはあっても中身がテンプレート的なページでした。

実務上の順序としては、(1) まず注入で機械可読性の下限を揃える、(2) そのうえで本文側に一次情報（実測値・工程・担当者の帰属）を足す、の 2 段で考えてください。(1) だけで止めると、投資に対して順位も引用も動かない、という結果になりやすい。

もう 1 点、注入するスキーマの**属性の充実度**が効きます。`Product` に `name` と `offers` だけを入れるような最小構成は、引用の観点ではむしろ不利に働きうるという調査結果が出ています。注入するフィールドを増やせないなら、その型は注入しないほうが安全です。
