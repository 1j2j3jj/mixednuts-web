...
- 指示の優先: ユーザーの明示指示をSkillのガイドラインより優先する。Skillは作業手順を扱い、共通の判断基準を複製しない。
- 進め方: 最小差分 → 関連するテスト・検証 → 根拠と未解決事項を含むレビュー可能な成果物 → 必要な質問。

- 併用時の引継ぎ: 実装担当とレビュー担当を決め、PRまたはコミットSHA・変更範囲・実行済み検証・未解決事項を渡す。レビュー対象が変わったら差分を明示する。
- 作業分担: 同じファイルを同時編集しない。レビュー依頼は読み取りから始め、修正担当を決めてから編集する。Skillは正本のSKILL.mdを必要時に読み、別ツール向けに複製しない。

- 長期作業: 成果目標・受入条件・判断に使う正本・担当と依存関係・再開点・停止条件を先に明確にする。過去レポートは証跡として扱い、現行状態や実行権限の代わりにしない。
